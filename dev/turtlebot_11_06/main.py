...
class Main():
    ...
    def handle_code(self):
        self.text_output.configure(text="Compiling code")
        try:
            #test if code compiles
            self.turtle.test_code(self.code_input.get_code(), self.text_output)
            if not self.text_output.cget("text") == "Compiling code":
                return
            #split code into  lines
            code_lines = self.code_input.get_code().splitlines()
            for_loop =0
            processed_lines = []
            preprocessing_lines = []
            ...
            for line in processed_lines:
                if self.turtle.get_paused():
                    break
                self.text_output.configure(text=line)
                self.turtle.run_code(line, self.text_output)
            if not self.turtle.get_paused():
                self.text_output.configure(text="Completed Successfully")
        except:
            pass
